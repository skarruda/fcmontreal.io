...
const MAX_MAIN = 24; // includes Luciano at slot 1, so 23 open spots
...
export default function App() {
  // mainPlayers = slots 2..24 (max 23 entries)
  ...
  const [listTarget,  setListTarget]    = useState("main"); // "main" | "wait"
  ...
  const mainFull = mainPlayers.length >= MAX_MAIN - 1; // -1 because Luciano holds slot 1

  // If main just filled up, auto-switch target to wait
  useEffect(() => {
  ...
  async function load() {
    try {
      ...
      if (g)  { const v = JSON.parse(g.value); setGameInfo(v); setEditGame(v); }
    } catch (_) {}
    setLoading(false);
  }
  ...
  function handleAdminLogin() {
    if (adminInput === ADMIN_PASSWORD) { setAdminAuth(true); setAdminInput(""); }
  ...
  const confirmedCount = 1 + mainPlayers.length; // Luciano + others
  ...
  const S = {
    ...
    emptySlot: {
      display:"flex", alignItems:"center",
      background:"transparent", border:"1px solid #0f1f12",
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:480, margin:"0 auto", padding:"24px 16px 56px" }}>
        ...
        <div style={{ display:"flex", gap:8, marginBottom:14 }}>
          {["main","wait"].map(t => {
            ...
            return (
              ...
              >
                {isMain
                  ? (mainFull ? "🔒 Main List Full" : "✅ Main List")
                  : "⏳ Waiting List"
                }
              </button>
            );
        ...
        <div style={{ display:"flex", gap:8, marginBottom:16 }}>
          <input
            ...
            onBlur={e  => (e.target.style.borderColor="#1e5228")}
          />
        ...
        {toast && (
          <div style={{
            ...
            marginBottom:16, textAlign:"center", animation:"fadeIn 0.2s ease",
          }}>{toast.msg}</div>
        )}

        {/* ── MAIN LIST ── */}
        <div style={{ marginBottom:20 }}>
          <h2 style={{ fontSize:13, fontWeight:700, color:"#4ade80", letterSpacing:2, textTransform:"uppercase", marginBottom:10, margin:"0 0 10px" }}>
            ✅ Main List
          </h2>
        ...
        <div style={{ marginBottom:24 }}>
          <h2 style={{ fontSize:13, fontWeight:700, color:"#fbbf24", letterSpacing:2, textTransform:"uppercase", margin:"0 0 10px" }}>
            ⏳ Waiting List
          </h2>
        ...
        <button onClick={() => setAdminOpen(!adminOpen)} style={{
          width:"100%", background:"transparent", border:"1px solid #1e5228",
          borderRadius:10, padding:10, color:"#4a7c59", fontSize:12,
        ...
        {adminOpen && !adminAuth && (
          <div style={{ display:"flex", gap:8, marginTop:8 }}>
            ...
            <button onClick={handleAdminLogin} style={{
              ...
              padding:"10px 16px", color:"#4ade80", fontWeight:700,
        ...
        {adminOpen && adminAuth && (
          <div style={{ marginTop:12, background:"#0a1f0d", border:"1px solid #1e5228", borderRadius:12, padding:16 }}>
